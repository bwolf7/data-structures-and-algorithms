...
const raisedToTheThird = (arr) => {
  ...
  let newArray = [];
  arr.forEach(number => {
    let powNum = Math.pow(number, 3)
    newArray.push(powNum)
  });
  return newArray
};
...
const addOne = (arr) => {
  // Solution code here...
  let newNumber = arr.map(number => number + 1)
  return newNumber
};
...
const addQuestion = (arr) => {
  // Solution code here...
  let newString = arr.map(string => string + "?")
  return newString
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let number of arr) {
    multArr.push(Math.pow(2, number))
  }
  return multArr
};
...
const mapTwoToThe = (arr) => {
  // Solution code here...
  let array3 = arr.map(number => Math.pow(2, number))
  return array3
};
